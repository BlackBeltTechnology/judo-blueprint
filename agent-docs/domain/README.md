...
The webshop is a **model-driven application** that serves a specific business purpose. This document outlines the core entities, relationships, and business rules that make up the domain model.
...
1.  **Consult the Model**: Always refer to `/model/webshop.model` as the single source of truth.
...
1. **Read the model first** - Always consult `/model/webshop.model` as the source of truth
