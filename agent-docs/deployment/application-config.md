...
The webshop application is configured through **environment variables** defined in the `judo-karaf.env` file. This approach ensures configuration is externalized and can be easily changed without modifying code or configuration files.

**Note**: Throughout this document, `webshop` refers to the application name from `judo.properties` (app_name property). Use this value in database names, Keycloak realms, and other application-specific settings.
...
| :--- | :--- | :--- |
| `JUDO_PLATFORM_DATASOURCE_URL` | `jdbc:postgresql://localhost:5432/webshop` | Database connection URL |
| `JUDO_PLAT-FORM_DATASOURCE_USERNAME`| `webshop` | Database username |
...
| `JUDO_PLATFORM_KEYCLOAK_REALM` | `webshop` | Keycloak realm name |
| `JUDO_PLATFORM_KEYCLOAK_RESOURCE` | `webshop-client` | Keycloak client ID |
...
JUDO_PLATFORM_DATASOURCE_URL=jdbc:postgresql://localhost:5432/webshop
JUDO_PLATFORM_DATASOURCE_USERNAME=webshop
JUDO_PLATFORM_DATASOURCE_PASSWORD=secret

# Keycloak configuration
JUDO_PLATFORM_KEYCLOAK_AUTH_SERVER_URL=http://localhost:8080/auth
JUDO_PLATFORM_KEYCLOAK_REALM=webshop
JUDO_PLATFORM_KEYCLOAK_RESOURCE=webshop-client
```
...
- **Example**: 
  - PostgreSQL: `jdbc:postgresql://localhost:5432/webshop`
  - HSQLDB: `jdbc:hsqldb:mem:webshop`
...
- **Example**: `noreply@webshop.example.com`
...
JUDO_PLATFORM_DATASOURCE_URL=jdbc:hsqldb:mem:webshop
...
# Database - PostgreSQL
JUDO_PLATFORM_RDBMS_DIALECT=postgresql
JUDO_PLATFORM_DATASOURCE_URL=jdbc:postgresql://db.example.com:5432/webshop
JUDO_PLATFORM_DATASOURCE_USERNAME=webshop_prod
...
JUDO_PLATFORM_KEYCLOAK_REALM=webshop-prod
JUDO_PLATFORM_KEYCLOAK_RESOURCE=webshop-prod-client
...
JUDO_PLATFORM_MAIL_FROM=noreply@webshop.example.com
```
...
**Test connection:**
```bash
psql -h localhost -U webshop -d webshop
```
...
```bash
curl http://localhost:8080/auth/realms/webshop
```
