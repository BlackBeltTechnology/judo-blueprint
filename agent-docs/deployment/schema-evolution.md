...
The `application/schema` module provides database schema migration capabilities for webshop. It uses Liquibase under the hood to perform incremental schema updates when the model evolves.

**Note**: Throughout this document, `webshop` refers to the application name from `judo.properties` (app_name property).
...
3.  **Apply the migration to your local database:**
    ...
    mvn judo-rdbms-schema:execute \
      -DbaseModelVersion=<base_version> \
      -DupdateModelVersion=<update_version> \
      -DjdbcUrl=jdbc:postgresql://localhost:5432/webshop \
      -DdbUser=webshop \
...
└── target/
    └── webshop-application-schema-[version].jar   # Executable JAR
```
...
# 3. Backup database
pg_dump -h localhost -U webshop webshop > backup_v1.1.0.sql
...
```bash
# 1. Stop database
docker stop webshop-postgres
# or: sudo systemctl stop postgresql

# 2. Remove database content
docker rm webshop-postgres
# or: dropdb webshop

# 3. Start fresh database
docker run -d --name webshop-postgres -e POSTGRES_DB=webshop -e POSTGRES_USER=webshop -e POSTGRES_PASSWORD=[password] -p 5432:5432 postgres:15

# 4. Restore from backup
psql -h localhost -U webshop -d webshop < backup_v1.1.0.sql
...
# Image created: webshop-application-schema:[version]
```
...
```bash
docker run --rm \
  webshop-application-schema:latest \
  execute \
  ...
  --jdbcUrl=jdbc:postgresql://host.docker.internal:5432/webshop \
  --dbType=postgresql \
  --dbUser=webshop \
  --dbPassword=[password]
```
...
services:
  db-migration:
    image: webshop-application-schema:${VERSION}
    command:
      - execute
      - --baseModelVersion=${BASE_VERSION}
      - --updateModelVersion=${UPDATE_VERSION}
      - --modelName=webshop
      - --jdbcUrl=jdbc:postgresql://postgres:5432/webshop
      - --dbType=postgresql
      - --dbUser=webshop
...
3. **Backup Production**
   ...
   pg_dump -h prod-db -U webshop webshop > prod_backup_$(date +%Y%m%d_%H%M%S).sql
   ```
...
mvn dependency:tree | grep webshop-model
```
...
migration-test:
  ...
  services:
    - postgres:15
  variables:
    POSTGRES_DB: webshop_test
    POSTGRES_USER: webshop
    POSTGRES_PASSWORD: test
  script:
    # Apply base schema
    - cd application/schema
    - mvn judo-rdbms-schema:apply -DjdbcUrl=jdbc:postgresql://postgres/webshop_test -DdbUser=webshop -DdbPassword=test
    # Generate migration
    - mvn judo-rdbms-schema:generate -DbaseModelVersion=${BASE_VERSION} -DupdateModelVersion=${CI_COMMIT_TAG}
    # Execute migration
    - mvn judo-rdbms-schema:execute -DbaseModelVersion=${BASE_VERSION} -DupdateModelVersion=${CI_COMMIT_TAG} -DjdbcUrl=jdbc:postgresql://postgres/webshop_test -DdbUser=webshop -DdbPassword=test
```
...
- Liquibase changelogs: `application/model/target/generated-resources/model/webshop-liquibase_[db].changelog.xml`
- Schema JAR: `application/schema/target/webshop-application-schema-[version].jar`
