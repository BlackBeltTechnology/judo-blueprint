...
1. **Task: Parse input and clone project**
   ...
   - `description`: "Extract the blueprint description and GitHub URL from arguments. Clone the project to /tmp."
   - `activeForm`: "Parsing input and cloning project"

2. **Task: Find model file and verify**
   - `subject`: "Find model file and verify"
   - `description`: "Locate the .model file in the cloned project. Verify it exists and the CLI can query it."
   - `activeForm`: "Finding model file"

3. **Task: Collect model blueprint**
   - `subject`: "Collect model blueprint"
...
/add-model-blueprint "User entity with role relation and permission flags" https://github.com/org/project
```
...
### Step 2: Find Model File and Verify

#### 2a. Find the Model File

```bash
find /tmp/judo-projects/{repo}/ -name "*.model" -type f 2>/dev/null | head -5
```

Store the model file path. If the URL pointed to a specific `.model` file, use that directly.

#### 2b. Verify CLI Access

Run a quick packages query to verify the model file is accessible:
```bash
...
If the CLI fails or no `.model` file exists, inform the user and stop.

### Step 3: Collect Model Blueprint
