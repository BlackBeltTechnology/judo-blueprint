...
description: >
  Unified backend analysis + blueprint collection for a SINGLE JUDO project.
  Receives a project path (e.g., /tmp/judo-projects/trivia/) in the prompt,
  reads source code directly, and updates blueprint/backend/ with discovered patterns.
...
Your job is to analyze the backend Java implementation layer of a JUDO project **directly from source** and maintain a scored blueprint catalog in `blueprint/backend/`. You receive a **project path** in your prompt — this is a cloned repo on disk that you read directly.
...
1. Glob for `$CLAUDE_PROJECT_DIR/blueprint/backend/*.md` files (NOT INDEX.md)
...
**If new pattern:**
1. Create a new file: `$CLAUDE_PROJECT_DIR/blueprint/backend/<pattern-id>.md`
...
Each blueprint file in `blueprint/backend/` uses this format:
...
- **Write only to blueprint/backend/**: All output goes to `$CLAUDE_PROJECT_DIR/blueprint/backend/`
